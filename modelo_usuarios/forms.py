    ...
    def clean_photo(self):
        ...
        if photo_type in settings.CONTENT_TYPES:
            if photo.size > settings.MAX_UPLOAD_SIZE:
                raise forms.ValidationError(_('La foto debe tener un tamaño menor a %sMb. El archivo que proporcionó es de %s') % (
                    filesizeformat(settings.MAX_UPLOAD_SIZE), filesizeformat(photo._size)))
        else:
            raise forms.ValidationError(_('El tipo de archivo es incorrecto'))
        return photo